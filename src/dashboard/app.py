...
from db.models import SessionLocal, Session, Trackpoint
...
load_dotenv()
st.set_page_config(page_title="Poseidon Dashboard", layout="wide", initial_sidebar_state="expanded")
...
def compute_derived_df(df_raw):
    ...
    df["speed_m_s"] = np.where(df["time_diff_s"] > 0, df["delta_dist"] / df["time_diff_s"], np.nan)
    df["speed_kmh"] = df["speed_m_s"] * 3.6
    df["pace_min_per_km"] = np.where(df["speed_m_s"] > 0, (1 / df["speed_m_s"]) / 60 * 1000, np.nan)
...
def detect_stable_segments(df, power_col="power_filtered", std_col="power_std",
                           min_power=50, std_threshold=5, min_duration_s=60):
    mask = (~df[power_col].isna()) & (df[power_col] >= min_power) & (df[std_col] <= std_threshold)
    segments = []
    current = None
...
def regression_with_ci(x, y, n_boot=200, ci=0.95):
    ...
    def percentile(arr, p):
        return np.nanpercentile(arr, [(1 - p) / 2 * 100, (1 + p) / 2 * 100]) if len(arr) > 0 else (np.nan, np.nan)
    ...
    return {
        "slope": coeffs[0],
        "intercept": coeffs[1],
        "r2": 1 - np.sum((ys - y_pred) ** 2) / np.sum((ys - np.mean(ys)) ** 2) if np.sum((ys - np.mean(ys)) ** 2) != 0 else np.nan,
        "x": xs,
        "y_pred": y_pred,
        "slope_ci": slope_ci,
...
def load_trackpoints(session_id):
    db = SessionLocal()
    try:
        return db.execute(select(Trackpoint).where(Trackpoint.session_id == session_id).order_by(Trackpoint.time)).scalars().all()
    finally:
        db.close()
...
def main():
    ...
    # kaleido warning
    if not HAVE_KALEIDO:
        st.warning("kaleido not installed: PDF export charts may fail. Install with `pip install -U kaleido`.")

    if "lang" not in st.session_state:
        st.session_state.lang = "en"
    lang_choice = st.sidebar.selectbox(f"{LANGUAGES[st.session_state.lang]['flag']} {LANGUAGES[st.session_state.lang]['language']}",
                                       ["en", "fr"], index=0 if st.session_state.lang == "en" else 1)
    ...
    primary_choice = st.sidebar.selectbox(TRANSLATIONS["primary_session"], keys, index=0)
    compare_choice = st.sidebar.selectbox(TRANSLATIONS["compare_to_optional"], ["None"] + keys, index=0)

    st.sidebar.markdown(f"### {TRANSLATIONS['stable_params']}")
    preset_names = list(st.session_state.presets.keys())
    selected_preset = None
    if preset_names:
        selected_preset = st.sidebar.selectbox(TRANSLATIONS["preset_select"], ["None"] + preset_names)
    ...
    max_power_threshold = st.sidebar.number_input(TRANSLATIONS["power_threshold"], value=preset.get("max_power_threshold", 250.0), step=10.0)
    min_stable_power = st.sidebar.number_input(TRANSLATIONS["min_stable_power"], value=preset.get("min_stable_power", 50.0), step=5.0)
    std_window_s = st.sidebar.number_input(TRANSLATIONS["rolling_std_window"], value=preset.get("std_window_s", 30), step=5)
    std_thresh = st.sidebar.number_input(TRANSLATIONS["std_threshold"], value=preset.get("std_thresh", 5.0), step=0.5)
    min_segment_duration = st.sidebar.number_input(TRANSLATIONS["min_segment_duration"], value=preset.get("min_segment_duration", 60), step=10)
    ...
    if preset_names:
        if st.button(TRANSLATIONS["preset_load"]) and selected_preset and selected_preset != "None":
    ...
    window_pts = max(1, int(round(std_window_s / median_dt))) if pd.notna(median_dt) and median_dt > 0 else 5
    ...
    reg_pc = regression_with_ci(df_primary["power_filtered"].to_numpy(), df_primary["cadence"].to_numpy())
    reg_ps = regression_with_ci(df_primary["power_filtered"].to_numpy(), df_primary["speed_kmh"].to_numpy())
    ...
    if compare_choice != "None" and compare_choice != primary_choice:
        ...
        if compare_session:
            ...
            df_compare["power_filtered"] = np.where(
                (~df_compare["power"].isna()) & (df_compare["power"] <= max_power_threshold),
                df_compare["power"],
                np.nan
            )
            median_dt_c = df_compare["time"].diff().dt.total_seconds().replace(0, np.nan).median()
            window_pts_c = max(1, int(round(std_window_s / median_dt_c))) if pd.notna(median_dt_c) and median_dt_c > 0 else 5
            df_compare["power_std"] = rolling_std(df_compare["power_filtered"], window_pts_c)
            reg_pc_comp = regression_with_ci(df_compare["power_filtered"].to_numpy(), df_compare["cadence"].to_numpy())
            reg_ps_comp = regression_with_ci(df_compare["power_filtered"].to_numpy(), df_compare["speed_kmh"].to_numpy())
    ...
    col2.metric(TRANSLATIONS["distance"], metric_value(primary_session.distance_km, "{:.2f}"))
    col3.metric(TRANSLATIONS["elevation_gain"], metric_value(primary_session.elevation_gain_m, "{:.1f}"))
    col4.metric(TRANSLATIONS["avg_speed"], metric_value(avg_speed_kmh, "{:.2f}"))

    extra1, extra2, extra3 = st.columns(3)
    extra1.metric(TRANSLATIONS["ftp_est"], metric_value(ftp_est, "{:.1f}"), help=TRANSLATIONS["tooltip_ftp"])
    extra2.metric(TRANSLATIONS["normalized_power"], metric_value(npower, "{:.1f}"), help=TRANSLATIONS["tooltip_np"])
    extra3.metric(TRANSLATIONS["tss"], metric_value(tss_val, "{:.1f}"), help=TRANSLATIONS["tooltip_tss"])

    total_pts = len(df_primary)
    kept = df_primary["power_filtered"].count()
    removed = total_pts - kept
    st.markdown(f"**{TRANSLATIONS['power_filtering']}:** " +
                TRANSLATIONS["filter_info"].format(threshold=max_power_threshold, removed=removed,
    ...
    with tab1:
        ...
        fig_power.add_trace(go.Scatter(
            x=df_primary["time"], y=df_primary["power"], name="Power raw", mode="lines",
            line=dict(color="lightgray"), opacity=0.6
        ))
        fig_power.add_trace(go.Scatter(
            x=df_primary["time"], y=df_primary["power_filtered"], name="Power filtered", mode="lines",
        ...
        if df_compare is not None:
            fig_power.add_trace(go.Scatter(
                x=df_compare["time"], y=df_compare["power_filtered"],
                name="Compare power filtered", mode="lines",
                line=dict(color="orange", dash="dash")
            ))
        fig_power.update_layout(title=TRANSLATIONS["power_over_time"], xaxis_title="Time", yaxis_title="Power (W)")
        st.plotly_chart(fig_power, use_container_width=True)

        if stable_segments:
            seg_labels = [f"{i+1}: {human_duration(s['duration_s'])} @ {s['avg_power']:.1f}W" for i, s in enumerate(stable_segments)]
            ...
            if sel != "None":
                ...
                fig_zoom.update_xaxes(range=[seg["start_time"] - pd.Timedelta(seconds=10),
        ...
        with cad_speed_col1:
            fig_cad = go.Figure()
            fig_cad.add_trace(go.Line(x=df_primary["time"], y=df_primary["cadence"], name=TRANSLATIONS["cadence_over_time"]))
            if df_compare is not None:
                fig_cad.add_trace(go.Line(x=df_compare["time"], y=df_compare["cadence"],
                                          name=f"Compare {TRANSLATIONS['cadence_over_time']}",
                                          line=dict(dash="dash", color="red")))
            fig_cad.update_layout(title=TRANSLATIONS["cadence_over_time"], xaxis_title="Time", yaxis_title="Cadence (rpm)")
            st.plotly_chart(fig_cad, use_container_width=True)
        with cad_speed_col2:
            fig_spd = go.Figure()
            fig_spd.add_trace(go.Line(x=df_primary["time"], y=df_primary["speed_kmh"], name=TRANSLATIONS["speed_over_time"]))
            if df_compare is not None:
                fig_spd.add_trace(go.Line(x=df_compare["time"], y=df_compare["speed_kmh"],
                                          name=f"Compare {TRANSLATIONS['speed_over_time']}",
                                          line=dict(dash="dash", color="red")))
            fig_spd.update_layout(title=TRANSLATIONS["speed_over_time"], xaxis_title="Time", yaxis_title="Speed (km/h)")
            st.plotly_chart(fig_spd, use_container_width=True)

        st.markdown("### Distributions")
        hist_col1, hist_col2 = st.columns(2)
        with hist_col1:
            fig_hist_power = px.histogram(df_primary, x="power_filtered", nbins=40, title="Power Filtered Distribution")
            st.plotly_chart(fig_hist_power, use_container_width=True)
        with hist_col2:
            fig_hist_cadence = px.histogram(df_primary, x="cadence", nbins=30, title="Cadence Distribution")
        ...
        with corr1:
            ...
            fig_pc = go.Figure()
            fig_pc.add_trace(go.Scatter(x=df_primary["power_filtered"], y=df_primary["cadence"],
                                        mode="markers", name="Primary", marker=dict(size=6, opacity=0.6)))
            if reg_pc:
                fig_pc.add_trace(go.Line(x=reg_pc["x"], y=reg_pc["y_pred"],
                                         name=f"Fit primary (R²={reg_pc['r2']:.3f})", line=dict(color="black")))
                fig_pc.add_annotation(x=np.nanmean(reg_pc["x"]), y=np.nanmean(reg_pc["y_pred"]),
                                      text=f"slope={reg_pc['slope']:.3f}, intercept={reg_pc['intercept']:.1f}",
                                      showarrow=False, bgcolor="white")
            if df_compare is not None and reg_pc_comp:
                fig_pc.add_trace(go.Scatter(x=df_compare["power_filtered"], y=df_compare["cadence"],
                                            mode="markers", name="Compare", marker=dict(size=6, opacity=0.4)))
                fig_pc.add_trace(go.Line(x=reg_pc_comp["x"], y=reg_pc_comp["y_pred"],
                                         name=f"Fit compare (R²={reg_pc_comp['r2']:.3f})", line=dict(dash="dash")))
            fig_pc.update_layout(xaxis_title="Power filtered (W)", yaxis_title="Cadence (rpm)")
            st.plotly_chart(fig_pc, use_container_width=True)
        with corr2:
            st.write(TRANSLATIONS["power_vs_speed"])
            fig_ps = go.Figure()
            fig_ps.add_trace(go.Scatter(x=df_primary["power_filtered"], y=df_primary["speed_kmh"],
                                        mode="markers", name="Primary", marker=dict(size=6, opacity=0.6)))
            if reg_ps:
                fig_ps.add_trace(go.Line(x=reg_ps["x"], y=reg_ps["y_pred"],
                                         name=f"Fit primary (R²={reg_ps['r2']:.3f})", line=dict(color="black")))
                fig_ps.add_annotation(x=np.nanmean(reg_ps["x"]), y=np.nanmean(reg_ps["y_pred"]),
                                      text=f"slope={reg_ps['slope']:.4f}, intercept={reg_ps['intercept']:.1f}",
                                      showarrow=False, bgcolor="white")
            if df_compare is not None and reg_ps_comp:
                fig_ps.add_trace(go.Scatter(x=df_compare["power_filtered"], y=df_compare["speed_kmh"],
                                            mode="markers", name="Compare", marker=dict(size=6, opacity=0.4)))
                fig_ps.add_trace(go.Line(x=reg_ps_comp["x"], y=reg_ps_comp["y_pred"],
                                         name=f"Fit compare (R²={reg_ps_comp['r2']:.3f})", line=dict(dash="dash")))
            fig_ps.update_layout(xaxis_title="Power filtered (W)", yaxis_title="Speed (km/h)")
        ...
        cleaned = df_primary[[
            "time", "power", "power_filtered", "cadence", "speed_kmh", "pace_min_per_km", "altitude_m", "distance_m"
        ]]
        csv_bytes = cleaned.to_csv(index=False).encode("utf-8")
        st.download_button(TRANSLATIONS["cleaned_trackpoints"], data=csv_bytes,
                           file_name=f"session_{primary_session.id}_cleaned.csv", mime="text/csv")

        if st.button(TRANSLATIONS["download_pdf"]):
            pdf_bytes = make_pdf(primary_session, df_primary, stable_segments,
                                 reg_pc, reg_ps, compare_session, df_compare,
                                 reg_pc_comp, reg_ps_comp, avg_speed_kmh, ftp_est, npower, tss_val, TRANSLATIONS)
            st.download_button("📄 PDF Report", data=pdf_bytes, file_name=f"session_{primary_session.id}_report.pdf", mime="application/pdf")
        ...
        if compare_session:
            ...
            with comp1:
                ...
                st.write(f"- {TRANSLATIONS['duration']}: {human_duration(primary_session.duration_s)}")
                st.write(f"- {TRANSLATIONS['distance']}: {metric_value(primary_session.distance_km, '{:.2f}')} km")
                st.write(f"- {TRANSLATIONS['avg_speed']}: {metric_value(avg_speed_kmh, '{:.2f}')} km/h")
            with comp2:
                st.subheader(TRANSLATIONS["compare"])
                st.write(f"- {TRANSLATIONS['duration']}: {human_duration(compare_session.duration_s)}")
                st.write(f"- {TRANSLATIONS['distance']}: {metric_value(compare_session.distance_km, '{:.2f}')} km")
                avg_speed_cmp = to_number(compare_session.avg_speed_kmh)
                if avg_speed_cmp is None or np.isnan(avg_speed_cmp):
                    if df_compare is not None and not df_compare["speed_kmh"].dropna().empty:
                        avg_speed_cmp = df_compare["speed_kmh"].dropna().mean()
                st.write(f"- {TRANSLATIONS['avg_speed']}: {metric_value(avg_speed_cmp, '{:.2f}')} km/h")
            delta_distance = to_number(primary_session.distance_km) - to_number(compare_session.distance_km)
            delta_speed = avg_speed_kmh - (avg_speed_cmp or 0)
            st.markdown(f"**{TRANSLATIONS['delta_distance']}:** {metric_value(delta_distance, '{:.2f}')} km")
            st.markdown(f"**{TRANSLATIONS['delta_avg_speed']}:** {metric_value(delta_speed, '{:.2f}')} km/h")
    ...
    with tab2:
        ...
        for s in sessions:
            ...
            df["power_filtered"] = np.where((~df["power"].isna()) & (df["power"] <= max_power_threshold), df["power"], np.nan)
        ...
        if not all_records:
            ...
            fig_ftp = px.line(weekly, x="week", y="ftp", title=TRANSLATIONS["ftp_trend"], markers=True)
            fig_np = px.line(weekly, x="week", y="np", title=TRANSLATIONS["np_trend"], markers=True)
            fig_tss = px.bar(weekly, x="week", y="tss", title=TRANSLATIONS["training_load"])
            st.plotly_chart(fig_ftp, use_container_width=True)
            st.plotly_chart(fig_np, use_container_width=True)
            st.plotly_chart(fig_tss, use_container_width=True)

def make_pdf(primary_session, df_primary, stable_segments,
             reg_pc, reg_ps, compare_session, df_compare,
             reg_pc_comp, reg_ps_comp,
             avg_speed_kmh, ftp_est, npower, tss_val, TRANSLATIONS):
    def sanitize_text(s: str) -> str:
        if not isinstance(s, str):
            return s
        replacements = {
    ...
    pdf.cell(0, 8, sanitize_text(f"{TRANSLATIONS['primary_session']}: {primary_session.id}"), ln=True)
    pdf.ln(2)
    pdf.set_font("Arial", "B", 12)
    pdf.cell(0, 6, "Key Metrics", ln=True)
    pdf.set_font("Arial", size=10)
    pdf.cell(0, 5, sanitize_text(f"{TRANSLATIONS['duration']}: {human_duration(primary_session.duration_s)}"), ln=True)
    pdf.cell(0, 5, sanitize_text(f"{TRANSLATIONS['distance']}: {metric_value(primary_session.distance_km, '{:.2f}')} km"), ln=True)
    pdf.cell(0, 5, sanitize_text(f"{TRANSLATIONS['avg_speed']}: {metric_value(avg_speed_kmh, '{:.2f}')} km/h"), ln=True)
    pdf.cell(0, 5, sanitize_text(f"{TRANSLATIONS['ftp_est']}: {metric_value(ftp_est, '{:.1f}')}"), ln=True)
    pdf.cell(0, 5, sanitize_text(f"{TRANSLATIONS['normalized_power']}: {metric_value(npower, '{:.1f}')}"), ln=True)
    pdf.cell(0, 5, sanitize_text(f"{TRANSLATIONS['tss']}: {metric_value(tss_val, '{:.1f}')}"), ln=True)
    pdf.ln(5)

    try:
        fig_power = go.Figure()
        fig_power.add_trace(go.Scatter(x=df_primary["time"], y=df_primary["power_filtered"], name="Power filtered", mode="lines"))
        for seg in stable_segments:
            fig_power.add_vrect(x0=seg["start_time"], x1=seg["end_time"], fillcolor="green", opacity=0.15, line_width=0)
        fig_power.update_layout(title=TRANSLATIONS["power_over_time"], xaxis_title="Time", yaxis_title="Power (W)")
        img_power = pio.to_image(fig_power, format="png", width=700, height=300)
        pdf.image(io.BytesIO(img_power), x=10, w=190)
    except Exception as e:
        pdf.set_font("Arial", "B", 12)
        pdf.cell(0, 6, sanitize_text(f"{TRANSLATIONS['power_over_time']} (image unavailable)"), ln=True)
