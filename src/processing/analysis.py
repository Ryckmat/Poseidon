...
from db.models import SessionLocal, Session, Trackpoint, StableSegment, Regression
...
def run_analysis(session_id):
    ...
    try:
        ...
        power = np.array([float(tp.power) if tp.power is not None else np.nan for tp in tps])
        cadence = np.array([float(tp.cadence) if tp.cadence is not None else np.nan for tp in tps])
        ...
        speed_m_s = np.divide(delta_dist, time_diff_s, out=np.zeros_like(delta_dist), where=time_diff_s>0)
        ...
        power_filtered = np.where((~np.isnan(power)) & (power <= max_power), power, np.nan)

        # Rolling std over approx 30s window (assume roughly uniform sampling)
        # estimate window size in points
        median_dt = np.median(time_diff_s[time_diff_s > 0]) if np.any(time_diff_s > 0) else 1
        ...
        stable_mask = (~np.isnan(power_filtered)) & (power_filtered >= 50) & (power_std <= std_thresh)
        segments = []
        current = None
        ...
        for i, tp in enumerate(tps):
            tp.speed_calc_kmh = float(speed_kmh[i]) if not np.isnan(speed_kmh[i]) else None
            tp.pace_min_per_km = float(pace[i]) if not np.isnan(pace[i]) else None
            tp.elevation_diff = float(elev_diff[i]) if not np.isnan(elev_diff[i]) else None
            tp.power_filtered = float(power_filtered[i]) if not np.isnan(power_filtered[i]) else None
...
if __name__ == "__main__":
    ...
    parser.add_argument("--session-id", required=True, help="UUID of session to analyze")
